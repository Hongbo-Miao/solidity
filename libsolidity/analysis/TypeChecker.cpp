...
Type const* const& TypeChecker::type(Expression const& _expression) const
{
	solAssert(!!_expression.annotation().type, "Type requested but not present.");
	return _expression.annotation().type;
}

Type const* const& TypeChecker::type(VariableDeclaration const& _variable) const
{
...
void TypeChecker::endVisit(Return const& _return)
{
	...
	{
		Type const* const& expected = type(*params->parameters().front());
...
bool TypeChecker::visit(VariableDeclarationStatement const& _statement)
{
	...
	for (size_t i = 0; i < min(variables.size(), valueTypes.size()); ++i)
	{
		...
		Type const* const& valueComponentType = valueTypes[i];
...
bool TypeChecker::visit(UnaryOperation const& _operation)
{
	...
	Type const* const& subExprType = type(_operation.subExpression());
...
{
	Type const* const& leftType = type(_operation.leftExpression());
	Type const* const& rightType = type(_operation.rightExpression());
...
)
{
	...
	Type const* const& expressionType = type(_functionCall.expression());
...
bool TypeChecker::visit(FunctionCall const& _functionCall)
{
	...
	case Type::Category::TypeType:
	{
		...
		Type const* const& actualType = dynamic_cast<TypeType const&>(*expressionType).actualType();
		solAssert(!!actualType, "");
